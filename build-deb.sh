...
#
# Adapted from Stenmark's build-deb.sh, contributed there by ner216 (Nolan
# Provencher) in https://github.com/mkay/stenmark/pull/3.
set -euo pipefail
...
# 2. Monaco is not in git. The fetch is a no-op once edith/data/monaco/vs exists.
bash scripts/fetch-monaco.sh >&2

# 3. Stage the application installation tree
rm -rf "$DEB_STAGING"
if [[ -d builddir ]]; then
...
# 4. Generate contents list for nFPM
CONTENTS=""
